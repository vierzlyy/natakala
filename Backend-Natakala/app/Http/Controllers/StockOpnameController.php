...
class StockOpnameController extends Controller
{
    ...
    public function pause(Request $request, StockOpnameSession $session): JsonResponse
    {
        if ($session->status !== 'open') {
            return response()->json(['message' => 'Hanya sesi aktif yang dapat dijeda.'], 422);
        }
        ...
        AuditLogger::write(
            ...
            $session->id,
            $session->session_no,
            'Sesi stock opname dijeda.',
        );

        return response()->json(['data' => $this->transform($session->fresh(['items.product']))]);
    }

    public function resume(Request $request, StockOpnameSession $session): JsonResponse
    {
        if ($session->status !== 'paused') {
            return response()->json(['message' => 'Hanya sesi yang dijeda yang dapat dilanjutkan.'], 422);
        }
        ...
        AuditLogger::write(
            $request->user(),
            'stock_opname_resumed',
            'stock_opname',
            $session->id,
            $session->session_no,
            'Sesi stock opname dilanjutkan kembali.',
        );

        return response()->json(['data' => $this->transform($session->fresh(['items.product']))]);
    }

    public function finalizeSession(Request $request, StockOpnameSession $session): JsonResponse
    {
        if (!$session->items()->exists()) {
            return response()->json(['message' => 'Belum ada item untuk diselesaikan.'], 422);
        }
        ...
        AuditLogger::write(
            $request->user(),
            'stock_opname_finalized',
            'stock_opname',
            $session->id,
            $session->session_no,
            'Sesi stock opname selesai dan masuk tahap review.',
        );
